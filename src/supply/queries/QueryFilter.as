package supply.queries {
	...
	import supply.queries.matchers.CompositeMatchAnyOf;
	...
	public class QueryFilter
	{
		...
		public function QueryFilter(query:Query, matchers:Vector.<IMatcher>, notWrapper:Boolean = false )
		{
			_query = query;
			_matchers = matchers;
		...
		public function values(...values) : Query
		{
			...
			if( _matchers.length == values.length )
			{
				...
				matcher = new CompositeMatchAllOf(_matchers);
			...
			if( _matchers.length == 1 ){
				...
				// append to query set below.
				matcher = new CompositeMatchAnyOf(_matchers);
